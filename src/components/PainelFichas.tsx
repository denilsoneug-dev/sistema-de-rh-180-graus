      ...
      <div className="card p-4 grid gap-3 sm:grid-cols-3">
        ...
        <div>
          ...
          <select id="filtro-status-rh" className="input" value={status} onChange={(e) => setStatus(e.target.value)}>
            ...
            {STATUS_RECRUTAMENTO.map((item) => <option key={item} value={item}>{STATUS_RECRUTAMENTO_LABELS[item]}</option>)}
          </select>
        </div>
        <div>
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                {podeEditar && (
                  <form action={atualizarStatusRecrutamento.bind(null, ficha.id)} className="flex gap-2">
                    ...
                    <select key={ficha.statusRecrutamento} id={`status-${ficha.id}`} name="status_recrutamento" defaultValue={ficha.statusRecrutamento} className="input !py-2 text-sm min-w-0">
                      {STATUS_RECRUTAMENTO.map((item) => <option key={item} value={item}>{STATUS_RECRUTAMENTO_LABELS[item]}</option>)}
                    </select>
